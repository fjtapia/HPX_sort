...
/// @remarks
//-----------------------------------------------------------------------------
#ifndef __HPX_PARALLEL_SORT_DETAIL_UTIL_MERGE_FOUR_HPP
#define __HPX_PARALLEL_SORT_DETAIL_UTIL_MERGE_FOUR_HPP
...
#include <hpx/parallel/sort/detail/util/range.hpp>
...
template < class Iter_t,
           class Compare = typename compare_iter < Iter_t>::value_type >
...
{   //---------------------------- begin ------------------------------------
	...
	uint32_t i = 0 ;
	while (i < nrange_input )
	{ 	if (vrange_input[i].size() != 0 ) ndest += vrange_input[i++].size() ;
		else
		{	for ( uint32_t k = i+1 ; k < nrange_input ; ++k)
			{	vrange_input[k-1] = vrange_input[k] ;
			};
    ...
    if ( pos[0]< pos[1])
    {	return concat (raux1,uninit_full_merge ( raux2, vrange_input[pos[0]],
    		                                     vrange_input[pos[1]], comp));
    }
    else
    {	return concat(raux1,uninit_full_merge ( raux2, vrange_input[pos[1]],
    		                                    vrange_input[pos[0]], comp));
    };
};
