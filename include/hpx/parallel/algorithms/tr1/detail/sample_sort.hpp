...
/// @remarks
//-----------------------------------------------------------------------------
#ifndef __HPX_PARALLEL_SORT_DETAIL_SAMPLE_SORT_HPP
#define __HPX_PARALLEL_SORT_DETAIL_SAMPLE_SORT_HPP
...
#include <algorithm>
#include <hpx/parallel/algorithms/tr1/detail/util/atomic.hpp>
#include <hpx/parallel/algorithms/tr1/detail/spin_sort.hpp>
#include <hpx/parallel/algorithms/tr1/detail/util/range.hpp>
#include <hpx/parallel/algorithms/tr1/detail/util/merge_four.hpp>
#include <hpx/parallel/algorithms/tr1/detail/util/merge_vector.hpp>
...
{   //---------------------------------- begin -------------------------------
    ...
    {   vfuture[i]=hpx::async (&this_t::execute_first,this);
    };
...
{   //---------------------------------- begin -------------------------------
    ...
    std::vector <hpx::future <void> > vfuture ( nthread ) ;
    for ( uint32_t i =0 ; i < nthread ; ++i)
    {   vfuture[i] = hpx::async ( &this_t::execute , this);
    };
    for ( uint32_t i =0 ; i < nthread ; ++i) vfuture[i].get() ;
};
...
{   //--------------------------- begin --------------------------------------
    ...
    //------------------------------------------------------------------------
    std::vector< std::vector<util::range <Iter_t> > > vv_range_first  (nthread);
