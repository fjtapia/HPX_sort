...
using hpx_det::block_indirect_sort ;
...
{   //-------------- begin------------
  std::vector<uint32_t > VN ;
    ...
    hpx_det::block_indirect_sort <4,4, uint32_t *, std::less<uint32_t> > BIS(&VN[0], &VN[130], comp,2);
};
...
{   //---------------------------- Inicio --------------------------------
    ...
    hpx_det::block_indirect_sort <4,4, uint64_t *, std::less<uint64_t> > BIS(&V1[0] , &V1[200], comp,2);
    ...
    hpx_det::block_indirect_sort<4,4, uint64_t *, std::less<uint64_t> > BI2(&V1[0] , &V1[200], comp,2);
    ...
    hpx_det::block_indirect_sort <4,4, uint64_t *, std::less<uint64_t> > BI3(&V1[0] , &V1[300], comp,2);
    ...
    hpx_det::block_indirect_sort<4,4, uint64_t *, std::less<uint64_t> > BI4(&V1[0] , &V1[300], comp,2);
    for ( unsigned i = 1 ; i < 300 ; i ++ )
    {	assert ( V1[i-1] <= V1[i] ) ;
    };
    return ;
...
{   //----------------------------- begin-------------------------------------
  ...
  typedef std::less<uint64_t> compare;
    ...
    hpx_det::block_indirect_sort <1024,16, iter_t, compare>  BI1(V1.begin() , V1.end(), comp,2);
    ...
    hpx_det::block_indirect_sort <1024,16, iter_t, compare>  BI2(V1.begin() , V1.end(), comp,2);
    ...
    hpx_det::block_indirect_sort <1024,16, iter_t, compare>  BI3(V1.begin() , V1.end() , comp,2);
    for ( unsigned i = 1 ; i < NElem ; i ++ )
    {	assert ( V1[i-1] <= V1[i] ) ;
    };
    ...
    hpx_det::block_indirect_sort <1024,16, iter_t, compare>  BI4(V1.begin() , V1.end(), comp,2);
    for ( unsigned i = 1 ; i < NElem ; i ++ )
    {	assert ( V1[i-1] == V1[i] ) ;
    };
};
...
{   //------------------------------------ begin -----------------------------
  ...
  typedef std::less<uint64_t> compare;
    ...
    hpx_det::block_indirect_sort<1024,32, iter_t, compare>  (V.begin() , V.end(),8);
...
{   //---------------------- begin ------------------------------------
    ...
    hpx_det::block_indirect_sort <1024,32, iter_t, compare>(A.begin() + 1000, A.begin() + (1000+NELEM), comp);
    ...
    hpx_det::block_indirect_sort <1024,32, iter_t, compare>(A.begin() + 1000, A.begin() + (1000+NELEM), comp,4);
...
{   //----------------------------- begin-------------------------------------
  ...
  typedef std::less<uint32_t> compare;
    ...
    V1 = V3 ;
    //std::cout<<"smart_merge_sort unsorted ---------------\n";
    hpx_det::block_indirect_sort<1024,32, iter_t, compare>(V1.begin() , V1.end(), 2);
    for ( unsigned i = 1 ; i < NElem ; i ++ )
    {	assert ( V1[i] == V2[i] )  ;
    };
    ...
    V1 = V3 ;
    //std::cout<<"smart_merge_sort unsorted ---------------\n";
    hpx_det::block_indirect_sort <1024,32, iter_t, compare> (V1.begin() , V1.end(), 4);
    for ( unsigned i = 1 ; i < NElem ; i ++ )
    {	assert ( V1[i] == V2[i] )  ;
    };
    ...
    V1 = V3 ;
    //std::cout<<"smart_merge_sort unsorted ---------------\n";
    hpx_det::block_indirect_sort <1024,32, iter_t, compare>(V1.begin() , V1.end(), 8);
    for ( unsigned i = 1 ; i < NElem ; i ++ )
    {	assert ( V1[i] == V2[i] )  ;
    };
    ...
    hpx_det::block_indirect_sort<1024,32, iter_t, compare> (V1.begin() , V1.end(),16);
    for ( unsigned i = 1 ; i < NElem ; i ++ )
    {	assert ( V1[i] == V2[i] )  ;
    };
    ...
    V1 = V3 ;
    //std::cout<<"smart_merge_sort unsorted ---------------\n";
    hpx_det::block_indirect_sort<1024,32, iter_t, compare> (V1.begin() , V1.end(),100);
    for ( unsigned i = 1 ; i < NElem ; i ++ )
    {	assert ( V1[i] == V2[i] )  ;
    };
...
{   //--------------------------- Inicio --------------------------
  ...
  hpx_det::block_indirect_sort <1024,32, iter_t, compare>(A.begin() , A.end(),comp);
