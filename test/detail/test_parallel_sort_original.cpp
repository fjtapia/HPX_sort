...
void test4()
{
	...
	V1 = V2;
	hpx_par::sort (hpx::parallel::par, p64, p64 + NELEM, cmp64);
	for (unsigned i = 1; i < NELEM; i++)
	{
	...
	V1 = V2;
	hpx_par::sort (hpx::parallel::par, p32, p32 + (NELEM << 1), cmp32);
	...
	V1 = V2;
	hpx_par::sort (hpx::parallel::par, p16, p16 + (NELEM << 2), cmp16);
	...
	V1 = V2;
	hpx_par::sort (hpx::parallel::par, p8, p8 + (NELEM << 3), cmp8);
