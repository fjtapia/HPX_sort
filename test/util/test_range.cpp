...
{   //-------------------------------------- begin --------------------------
    ...
    typedef hpx_util::range<fiter_t>  frange_t ;
    ...
    F1 = hpx_util::uninit_move ( F1, frange_t(V.begin(),V.end()));
    for ( uint32_t i =0 ; i < 10 ; ++i)
        assert ( PAux[i].N == i) ;

    hpx_util::destroy <forensic *>( F1);
    //for ( uint32_t i =0 ; i < 10 ; ++i)
    //{   if ( PAux[i].N != -1)
